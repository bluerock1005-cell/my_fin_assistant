...
> 适用：本项目所有 `features/*/<模块>_ui.py` 的桌面端（PySide6）界面。
> 参考实现（基准）：`features/notes_receivable_import/notes_receivable_import_ui.py`（应收票据批量导入）。
> 已对齐实例：`features/bank_classify/bank_classify_ui.py`（票据分类，已按本规范改造）。
...
- **UI / 逻辑严格分离**：`<模块>_ui.py` 只管界面与交互；重计算、文件 IO、Excel 读写放 `*_logic.py`（零 UI 依赖、可单独测试）。
...
- ❌ 在 `<模块>_ui.py` 里做重计算或文件 IO（应放 `*_logic.py`）。
